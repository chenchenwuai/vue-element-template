  ...
  const data = {
    ...
    status: {
      ...
      get isCUing() {
        ...
        throw new Error('wrong crud\'s isCUing status')
      },
      // 标题
      get title() {
  ...
  const methods = {
    ...
    refresh() {
      ...
      return new Promise((resolve, reject) => {
        ...
        initData(crud.url, crud.getQueryParams()).then(data => {
          ...
          if ((typeof Array.isArray === 'undefined' && Object.prototype.toString.call(data) === '[object Array]') || Array.isArray(data)) {
            crud.data = JSON.parse(JSON.stringify(data)) || []
          } else {
            crud.page.total = data.totalElements || 0
            crud.data = JSON.parse(JSON.stringify(data.content)) || []
          }
    ...
    findVM(type) {
      try {
        ...
        throw new Error('Not Find ' + type + ' vm')
      }
    },
...
function mergeOptions(src, opts) {
  ...
  for (const key in opts) {
    // eslint-disable-next-line no-prototype-builtins
    if (optsRet.hasOwnProperty(key)) {
      if (Array.isArray(opts[key])) {
        optsRet[key] = opts[key].slice()
